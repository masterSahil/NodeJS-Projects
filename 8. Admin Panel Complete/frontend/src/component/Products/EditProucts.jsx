...
import { Layers3, Save, IndianRupee, UploadCloud, Trash2 } from "lucide-react"; 
  ...
  const fetchProductDetails = async () => {
    try {
      ...
      if (product.image) {
        ...
        setImagePreview(`http://localhost:9000/uploads/${product.image}`);
      }
    } catch (error) {
      console.log(error);
      Swal.fire({
        ...
        <div className="max-w-7xl mx-auto bg-white rounded-2xl border border-gray-100 shadow-sm p-8 transition-all">
          ...
          <div className="flex items-center gap-4 mb-8 pb-5 border-b border-gray-50">
            ...
            <div>
              ...
              <p className="text-sm text-gray-500 mt-0.5">
                Update the configuration for this product.
              </p>
            </div>

            <div className="pt-4 ml-auto border-t justify-end border-gray-100">
              <button onClick={handleSubmit} type="button" className="w-full md:w-auto md:px-10 md:float-right inline-flex items-center justify-center gap-2 bg-indigo-600 hover:bg-indigo-700 active:bg-indigo-800 text-white text-sm font-medium py-3 rounded-md transition-all shadow-sm hover:shadow">
                <Save size={18} />
                Update Product
              </button>
            </div>
          ...
          <form onSubmit={handleSubmit} className="space-y-6">
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              ...
              {/* Left Column: Category Selectors & Pricing */}
              <div className="space-y-6">
                ...
                <div>
                  <label className="block text-sm font-medium text-gray-700 mb-2">Select Extra Sub Category (Name)</label>
