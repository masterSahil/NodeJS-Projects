...
import { Search, Plus, MoreVertical, LayoutGrid, Table2, Edit, Trash2, FolderTree } from "lucide-react";
...
export default function ViewSubCategories() {
  ...
  const toggleStatus = async (id, currentStatus) => {
    try {
      await axios.put(`http://localhost:9000/subcategory/${id}`,
        { isActive: !currentStatus });

      setSubCategories((prev) => prev.map((item) =>item._id === id?{...item, isActive: !currentStatus}: item));
      Swal.fire({
        icon: "success",
        title: "Updated",
        text: `Sub Category ${ !currentStatus ? "Activated" : "Deactivated" }`,
        timer: 1500,
      ...
      Swal.fire({
        icon: "error",
        title: "Error",
        text: "Failed to Update Category Toggle",
      });
    }
  };
  ...
  const filteredSubCategories =
    subCategories.filter((item) => {
      ...
      return (sub.includes(searchTerm.toLowerCase()) ||
      ...
      <main className="max-w-7xl mx-auto flex-1 overflow-y-auto p-6 lg:p-8">
        ...
        <div className="bg-white p-4 rounded-lg border border-gray-200 shadow-sm mb-6">
          <div className="flex flex-col lg:flex-row lg:items-center justify-between gap-4">
            <div className="relative w-full lg:max-w-sm">
              ...
              <input type="text" placeholder="Search sub category..." value={searchTerm}
            ...
            <div className="flex items-center gap-2">
              ...
              <button onClick={() => setViewType("grid")} className={`p-3 rounded-md 
                  ...
                  {filteredSubCategories.map(
                    (item) => (
                      <tr key={item._id} className="border-b border-gray-100 hover:bg-gray-50">
                        ...
                        <td className="px-6 py-5 font-medium"> { item.categoryId ?.category } </td>
                        ...
                        <td className="px-6 py-5">
                          <div className="flex justify-end gap-3">
                            <Link to={`/sub-categories/edit/${item._id}`}
        ...
        {viewType === "grid" && (
          <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-6">
            {filteredSubCategories.map(
              (item) => (
                <div key={item._id} className="bg-white border border-gray-200 rounded-xl p-6 shadow-sm hover:shadow-md relative">
                  ...
                  <h2 className="text-xl font-bold mt-5"> {item.subcategory} </h2>
                  ...
                  {openMenuId === item._id && (
                    <>
                      ...
                      <div className="absolute top-20 right-5 w-40 bg-white border rounded-lg shadow-lg z-20">
                        ...
                        <button onClick={() => handleDelete( item._id )}
                          className="w-full flex items-center gap-2 px-4 py-3 text-red-600 border-t-black border-t">
